...
t2_df = pd.DataFrame({'Timestamp': t2_counts.index, 'Count': t2_counts.values})

# Create new histogram for T2
t2_hist = px.bar(t2_df, x='Timestamp', y='Count', 
                 title='Number of Records per T2 Timestamp',
                 labels={'Timestamp': 'T2 Timestamp', 'Count': 'Number of Records'},
                 color='Count', color_continuous_scale=px.colors.sequential.Viridis)

t2_hist.update_layout(
    xaxis_title='T2 Timestamp',
...
def toggle_content(n_clicks, current_content):
    if n_clicks % 2 == 0:
        ...
        return html.Div([
            html.H2("T2 Timestamp Analysis", style={'color': '#34495e', 'textAlign': 'center'}),
