...
def process_event_data(option_emm_id, underlying_emm_id, event, seen_keys):
    ...
    def safe_time_diff(end, start):
        return max(0, nanoseconds_to_seconds(end - start))
    
    diffs = [
        safe_time_diff(event[1], event[0]),
        safe_time_diff(event[2], event[1]),
        safe_time_diff(event[3], event[2]),
        safe_time_diff(event[4], event[3])
    ]
    t5_t2_diff = safe_time_diff(event[4], event[1])
